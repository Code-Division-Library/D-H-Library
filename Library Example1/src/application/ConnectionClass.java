...
public class ConnectionClass {
    ...
    public ObservableList getAllBooks(String s) throws SQLException {
        ...
        while (rSet.next()) {
            list.add(new Stock(rSet.getString("Title"), rSet.getString("Author"), rSet.getString("Category"), rSet.getString("Shelf-ID"), rSet.getInt("Available")));
        }
        rSet.close();
        stat.close();
    ...
    public ObservableList getAllJournals(String s) throws SQLException {
        ObservableList list = FXCollections.observableArrayList();
        Statement stat = myCon.createStatement();
        if (s.equals("All")) {
            s = "SELECT * FROM journals;";
        } else {
            s = "SELECT * FROM journals WHERE category=\"" + s + "\";";
        }
        ResultSet rSet = stat.executeQuery(s);
        while (rSet.next()) {
            list.add(new Stock(rSet.getString("Title"), rSet.getString("Volume"), rSet.getString("Country"), rSet.getString("Shelf-ID"), rSet.getInt("Available")));
        }
        rSet.close();
        stat.close();
    ...
    public void addJournal(String title, int volume, String country, String shelfID, String amount) throws SQLException {
        Statement state = myCon.createStatement();
        ResultSet resultSet = state.executeQuery("select `Shelf-ID`,`Available` from books where Title=\"" + title + "\" AND Volume=\"" + volume + "\"");
        int avail = 0;
        boolean done = false;
        while (resultSet.next()) {
            if (resultSet.getString("Shelf-ID").equals(shelfID)) {
                avail = resultSet.getInt("Available");
                amount = "" + (Integer.parseInt(amount) + avail);
                int rs = state.executeUpdate("update books set Available = " + amount + " where Title=\"" + title + "\" AND Volume=\"" + volume + "\" AND `Shelf-ID`=\"" + shelfID + "\";");
                done = true;
                break;
            }
        }
        if (!done) {
            int rs = state.executeUpdate("Insert Into books(`Title`, `Volume`, Country, `Shelf-ID`, `Available`)" +
                    " values (\"" + title + "\",\"" + volume + "\",\"" + country + "\",\"" + shelfID + "\"," + amount + ")");
        }
    ...
    public void remJournals(String title, int volume) throws SQLException {

        Statement state = myCon.createStatement();
        ResultSet res = state.executeQuery("select `Shelf-ID`,`Available` from journals where Title= \"" + title + "\" AND Volume= \"" + volume + "\"");
        ...
        shelfText.setFocusTraversable(false);
        TextField available = new TextField();
        available.setPromptText("Enter Number of Journals to Remove");
        available.setPrefSize(270, 35);
        available.setMaxSize(270, 35);
        available.setFocusTraversable(false);
        Button confirm = new Button("Confirm");
        ...
        confirm.setOnAction(event -> {
            ...
            if (rem < 0) {
                ...
                try {
                    ...
                    int a = state.executeUpdate("update journals set `Available`=" + rem + " where Title=\"" + title + "\" AND " +
                            "Volume=\"" + volume + "\" AND `Shelf-ID`=\"" + shelfText.getText() + "\"");
                    arg.close();
                } catch (SQLException e) {
                    e.printStackTrace();
                }
    ...
    public void IssueBook(int id, String title, String author, String shelfID, String amount, String memberID) throws SQLException {
        ...
        if (available >= Integer.parseInt(amount)) {
            ...
            int done = stat.executeUpdate("update journals set `Available`=" + (available - counter) + " where Title=\"" + title + "\" AND Author=\"" + author + "\" AND `Shelf-ID`=\"" + shelfID + "\";");
    ...
    public ObservableList getAddRemBookHistory(String search, String startDate, String endDate, String cat) throws SQLException {
        ...
        ResultSet rSet = stat.executeQuery(s);
        while (rSet.next()) {
            list.add(new Book(rSet.getString("Title"), rSet.getString("Author"), rSet.getDate("Date"), rSet.getString("Time"),
